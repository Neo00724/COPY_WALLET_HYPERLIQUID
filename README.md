...
| `change_threshold` (in `COPY_HL.py`)| 0.5 | Minimum position size or position change threshold (% of account). Will ignore changes, or positions with sizes below this %. If the trader does a lot few small changes below 0.5%, the bot will still trigger a position size change when relative "error"/difference between the expected and actual position is above 10% |
