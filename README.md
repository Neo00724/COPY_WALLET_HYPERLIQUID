...
**EDIT:** added a Long+Short version `COPY_HL_LS`, but default is Long-only. 
<br />You need to edit `docker-compose.yml` and replace `--strategy COPY_HL` by `--strategy COPY_HL_LS` to use it. Still experimental, use at your own risk. Dry-run (paper trading) is highly recommended.
